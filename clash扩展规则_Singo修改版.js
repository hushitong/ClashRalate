...
const ruleset_categorybankjp = {
  ...
  tolocalpath: "./ruleset/MetaCubeX/category-bank-jp.mrs"
}
...
const rules = [
  ...
  'DOMAIN-SUFFIX,girigirilove.com,DIRECT',  //girigirilove在线动画
  'DOMAIN-SUFFIX,zb21sajhh83230-9939.app,DIRECT',  //绿茵直播
  'DOMAIN-SUFFIX,ednovas.xyz,DIRECT',  //GitHub 文件加速
  'DOMAIN-SUFFIX,gh-proxy.com,DIRECT',  //GitHub 文件加速
  'DOMAIN-SUFFIX,1888866.xyz,DIRECT',  //GitHub 文件加速
...
const direct_nameserver = ['https://223.5.5.5/dns-query','https://doh.pub/dns-query','https://doh.360.cn/dns-query']
...
const dnsConfig = {
  ...
  'fake-ip-filter': ['*', '+.lan', '+.local', '+.market.xiaomi.com','geosite:connectivity-check','geosite:private'],

  'default-nameserver': [...defaultDNS],  // 用来解析「DNS 服务器域名」的 DNS，需要填 IP 地址
  'direct-nameserver': [...direct_nameserver],  // 「直连」域名的解析
  'proxy-server-nameserver': [...foreignDNS],  // 「代理服务器域名」域名的解析，防止 nameserver 无法访问导致连不上代理
  nameserver: [...foreignDNS],  // 用来解析没有匹配到任何「域名规则」的域名，通常是国外域名
...
function main(config, filename) {
  ...
  config['allow-lan'] = true  // 局域网连接

  config['bind-address'] = '*'

  config['mode'] = 'rule'  // 代理模式：规则
  ...
  config['geox-url'] = {
    geoip: geoxUrl.geoip,
    geosite: geoxUrl.geosite,
    mmdb: geoxUrl.country_lite,
    asn: geoxUrl.asn,
  }
  ...
  // 获得低倍率节点，这里直接硬对比了，实在是提供的名字千奇百怪，不好用正则匹配
  let proxiesLowRatio = config.proxies
    .filter((a) => {
      if (a.name === ("🇺🇸US 05 - 0.01x - Download") || a.name === "HK - 0.10  - FxT 测试")
        return true
      else
        return false
    })
  ...
  let lowRatioGroup = {}
  if (proxiesLowRatio.length > 0) {
    lowRatioGroup = ({
      ...groupBaseOption,
      name: "低倍率节点",
      type: 'select',
      icon: "https://fastly.jsdelivr.net/gh/Koolson/Qure@master/IconSet/Color/CreamData.png",
      proxies: proxiesLowRatio,
    })
  }
  ...
  if (ruleOptions.ads) {
    ...
    ruleProviders.set(ruleset_adblockmihomo.name, {
      ...ruleProviderCommon,
      behavior: ruleset_adblockmihomo.behavior,
      format: ruleset_adblockmihomo.format,
      url: ruleset_adblockmihomo.url,
      path: ruleset_adblockmihomo.tolocalpath,
    })
    config['proxy-groups'].push({
      ...groupBaseOption,
      name: '广告过滤',
  ...
  if (ruleOptions.youtube) {
    ...
    config['proxy-groups'].push({
      ...
      name: 'YouTube',
      type: 'select',
      proxies: ['默认节点', '直连', lowRatioGroup.name, ...proxyGroupsRegionNames],
  ...
  if (ruleOptions.japan) {
    ...
    ruleProviders.set(ruleset_categorybankjp.name, {
      ...ruleProviderCommon,
      behavior: ruleset_categorybankjp.behavior,
      format: ruleset_categorybankjp.format,
      url: ruleset_categorybankjp.url,
      path: ruleset_categorybankjp.tolocalpath,
    })
    config['proxy-groups'].push({
      ...groupBaseOption,
      name: '日本网站',
